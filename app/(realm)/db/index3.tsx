...
// 4. 创建数据展示组件
function DataList() {
  const [persons, setPersons] = useState<Realm.Results<PersonType> | null>(null);
  // 加载数据
  const loadData = async () => {
    const realm = await getDB();
    const persons = realm.objects<PersonType>('Person');
    setPersons(persons as Realm.Results<PersonType>);
  };
  ...
  const updateCar2 = async () => {
    try {
      ...
      realm.write(() => {
        ...
        if (person) {
          person.cars[0].name = '宝马999999';
          person.cars[0].brand = '宝马999999';
          person.cars[0].price = 999999;
        }
      })
    } catch (error) {
  ...
  return (
    <ScrollView style={{ padding: 16 }}>
      ...
      {persons && persons.map((person) => (
        <>
          <Text key={person._id}>{person.name}</Text>
          <View>
            {person.cars.map((car) => ( 
              <>
                <Text key={car._id}>{car.name}</Text>
                <Text>{car.brand}</Text>
                <Text>{car.price}</Text>
              </>
            ))}
